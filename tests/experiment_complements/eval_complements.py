#!/usr/bin/env python3
"""
Eval complements - 5 jutges (GPT-4o, Gemini Flash, Qwen, Llama, Mistral) amb exclusio auto-avaluacio.
"""
...
OUTPUT_PATH = EXP_DIR / "resultats_avaluacio_complements.json"
...
def build_eval_prompt(rubrica, complement, perfil_desc, mecr, tipus_nom, text_original):
    criteris_text = ""
...
## Tipus de complement avaluat
{tipus_nom}

## Criteris d'avaluació
{criteris_text}

## Text original (al qual acompanya el complement)
{text_original}

## Complement a avaluar
{complement}

## Format de resposta
Respon EXACTAMENT en JSON valid sense text fora:
{{
  "correccio_linguistica": {{"puntuacio": X, "justificacio": "..."}},
  "adequacio_perfil": {{"puntuacio": X, "justificacio": "..."}},
  "carrega_cognitiva": {{"puntuacio": X, "justificacio": "..."}},
  "utilitat_practica": {{"puntuacio": X, "justificacio": "..."}},
  "coherencia_text": {{"puntuacio": X, "justificacio": "..."}}
}}
"""
...
def main():
    print("=" * 60)
    print("EVAL COMPLEMENTS - 5 jutges")
    print("=" * 60)
    ...
    for model in ["gpt4o-mini", "mistral", "llama", "gemma"]:
        f = EXP_DIR / f"resultats_complements_{model}.json"
        if f.exists():
            d = json.load(open(f, "r", encoding="utf-8"))
            for c in d.get("casos", []):
                if not str(c.get("complement_generat", "")).startswith("ERROR"):
                    all_cases.append(c)
    ...
    for i, case in enumerate(all_cases):
        ...
        tipus = tipus_map[case["tipus_complement"]]

        eval_prompt = build_eval_prompt(
            rubrica,
            case["complement_generat"],
        ...
        for jutge_name in judges_to_use:
            try:
                result = JUDGES[jutge_name](eval_prompt, "Avalua el complement.")
        ...
        if existing_eval:
            ...
            evals.append({
                ...
                "tipus_complement": case["tipus_complement"],
        ...
        if (i + 1) % 5 == 0:
            output = {
                "experiment": "complements_eval_multi",
