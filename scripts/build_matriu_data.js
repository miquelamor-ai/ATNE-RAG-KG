 ...
 *
 * Font única preservada: el .data.js és DERIVAT del canon (no s'edita a mà).
 *
 * ┌─────────────────────────────────────────────────────────────────────────┐
 * │ FLUX OBLIGATORI després de bumpejar el submodule corpusFJE:              │
 * │                                                                          │
 * │   node scripts/build_matriu_data.js && node tests/test_complements_matriu.js │
 * │                                                                          │
 * │ El 1r pas regenera el derivat; el 2n el verifica (guard d'equivalència   │
 * │ .data.js ↔ matriu_cobertura.json + golden snapshot 711). Si el test KO   │
 * │ amb "DRIFT", és que t'has oblidat el 1r pas.                             │
 * └─────────────────────────────────────────────────────────────────────────┘
 */
'use strict';
const fs = require('fs');
